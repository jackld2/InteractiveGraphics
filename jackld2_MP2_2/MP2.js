...
//Identity Quaternion
var quatAngle = quat.create();
var turnSpeedasDeg = 4;
...
var newEyePt = vec3.fromValues(0.0,0.0,0.0);
var fogValue = 1.0;
...
function setupShaders() {
  ...
  shaderProgram.uniformFogLoc = gl.getUniformLocation(shaderProgram, "isFog");
 ...
 * @param {Float32} fog true or false 1 or 0
 */
function setFogUniforms(fogToggle) {
  gl.uniform1f(shaderProgram.uniformFogLoc, fogToggle);
}
...
function animate() {
    ...
    console.log("Eyepoint: ", eyePt);
    
    //Perform player movement
    newEyePt = viewDir;
    vec3.normalize(newEyePt, newEyePt);
    vec3.scaleAndAdd(eyePt, eyePt, newEyePt, planespeed);
...
function keyInput(event) {
    ...
    if(event.keyCode=="70") {
        ...
        // INCREASE PLANE SPEED, KEY +
    if (event.keyCode == "187") {
        planespeed += 0.0005;
        console.log("speed:", planespeed);
    }
    // DECREASE PLANE SPEED, KEY -
    if (event.keyCode == "189"  && planespeed > 0.0005) {
        planespeed -= 0.0005;
        console.log("speed:", planespeed);
    }

    // ROLL LEFT, KEY LEFT ARROW
    if (event.keyCode == "37") {
    //returns a quaternion with 
    quat.setAxisAngle(quatAngle, viewDir, -1*degToRad(turnSpeedasDeg) )
    vec3.transformQuat(Xaxis, Xaxis, quatAngle);
    vec3.transformQuat(up, up, quatAngle);
    }
    
    // ROLL RIGHT, KEY RIGHT ARROW
    if (event.keyCode == "39") {
    //returns a quaternion with 
    quat.setAxisAngle(quatAngle, viewDir, degToRad(turnSpeedasDeg) )
    vec3.transformQuat(Xaxis, Xaxis, quatAngle);
    vec3.transformQuat(up, up, quatAngle);
    }
    
        // PITCH UP, KEY UP ARROW
    if (event.keyCode == "38") {
    //returns a quaternion with 
    quat.setAxisAngle(quatAngle, Xaxis, degToRad(turnSpeedasDeg) )
    vec3.transformQuat(viewDir, viewDir, quatAngle);
    vec3.transformQuat(up, up, quatAngle);
    }
    // PITCH DOWN, KEY DOWN ARROW
    if (event.keyCode == "40") {
    //returns a quaternion with 
    quat.setAxisAngle(quatAngle, Xaxis, -1*degToRad(turnSpeedasDeg) )
    vec3.transformQuat(viewDir, viewDir, quatAngle);
    vec3.transformQuat(up, up, quatAngle);
    }
 ...
 */
 function startup() {
     ...
     //take input from a key being pressed
